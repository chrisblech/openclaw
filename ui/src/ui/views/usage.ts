...
function renderCodexLimitsCard(props: UsageProps) {
  const { codexLimits } = props;
  const snapshot = codexLimits.snapshot;
  const accounts = snapshot?.accounts ?? [];
  if (!codexLimits.loading && !codexLimits.error && accounts.length === 0) {
    return nothing;
  }
  ...
  const renderBar = (remaining: number | null) => {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center; gap: 12px;">
        ...
        <div style="display:flex; align-items:center; gap:10px;">
          ${codexLimits.loading ? html`<span style="color: var(--muted); font-size: 12px;">lädt…</span>` : nothing}
          ${codexLimits.error ? html`<span style="color: #ff4d4d; font-size: 12px;">${codexLimits.error}</span>` : nothing}
          <button class="btn btn--secondary" @click=${() => props.callbacks.filters.onRefresh()} title="Refresh">
            Refresh
          </button>
        </div>
      </div>
