...
export function renderUsageTab(state: AppViewState) {
  ...
  return renderUsage({
    ...
    codexLimits: {
      loading: state.codexLimitsLoading,
      snapshot: state.codexLimits,
      error: state.codexLimitsError,
    },
    filters: {
    ...
    callbacks: {
      filters: {
        ...
        onRefresh: () => {
          ...
          if (!state.codexLimits && !state.codexLimitsLoading) {
      ...
      details: {
        ...
        onSelectSession: (key, shiftKey) => {
          state.usageTimeSeries = null;
          state.usageSessionLogs = null;
          state.usageRecentSessions = [key, ...state.usageRecentSessions.filter((entry) => entry !== key)].slice(0, 8);
          ...
          } else if (state.usageSelectedSessions.length === 1 && state.usageSelectedSessions[0] === key) {
            state.usageSelectedSessions = [];
          } else {
